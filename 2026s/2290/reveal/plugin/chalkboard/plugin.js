...
/**************************************************************
 ** Modification by MCT on 2026-03-16
 ** New behavior: persist chalkboard across all fragments.
 ** 
 ** Change made:
 ** Commented out all instances of "storage[ id ].data[ i ].slide.f" "indices.f"
 ** 
 ** Why this works:
 ** A new chalkboard is used for each horizontal "h", vertical "v", and fragment "f" slide. 
 ** If we forget the fragment used, then each chalkboard is indexed by (h,v) not (h,v,f). 
***************************************************************/
	...
	function getSlideData( indices, id ) {
		...
		for ( var i = 0; i < storage[ id ].data.length; i++ ) {
			...
			if ( storage[ id ].data[ i ].slide.h === indices.h && storage[ id ].data[ i ].slide.v === indices.v) {
				// MCT: Console log to see what slide indices look like: "f" = "fragment", "h" = horizontal slides, "v" = vertical slides
				// console.log("Slide indices 'h': " +  storage[ id ].data[ i ].slide.h);
				// console.log("Slide indices 'v': " +  storage[ id ].data[ i ].slide.v);
				// console.log("Slide indices 'f': " +  storage[ id ].data[ i ].slide.f);
